...
int main(int argc, char **argv){
	//this is the size of a page of memory. Typically 4096 bytes
	const size_t PAGE_SIZE = sysconf(_SC_PAGE_SIZE);

	if (argc == 1){
		//No arguments were given, so print the usage text and exit; 
		//NOTE: The first argument is actually the program name, so argc[0]
	...
	// which ensures that the returned address is multiple of the page size
	// (4096 = 0x1000, so indeed, any address that is a multiple of 4096 will
	// have the last 3 nibbles equal to 0).
	uint32_t page_alligned_addr = ADDRESS & ~(PAGE_SIZE - 1);
	...
	uint32_t *page_virtual_addr = (uint32_t *)mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, page_alligned_addr);
	...
	// The address we want to access might not be page_alligned. Since we mapped
	// a page-alligned address, we need our target address' offset from the 
	...
	//outside our program; the address refers to memory-mapped I/O
